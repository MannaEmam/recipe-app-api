...
def detail_url(recipe_id):
    """Create adn return a recipe detail URL."""
...
class PrivateRecipeApiTests(TestCase):
    ...
    def test_create_recipe_with_existing_tags(self):
        ...
        tag_usa = Tag.objects.create(user=self.user, name="Usa")
        payload = {
            'title': 'Usa Prawn Curry',
            'time_minutes': 30,
            'price': Decimal('2.50'),
            'tags': [{'name': 'Usa'}, {'name': 'Breakfast'}]
        }
        res = self.client.post(RECIPES_URL, payload, format='json')
        ...
        recipe = recipes[0]
        self.assertEqual(recipe.tags.count(), 2)
        self.assertIn(tag_usa, recipe.tags.all())
        for tag in payload['tags']:
            exists = recipe.tags.filter(
                name=tag['name'],
                user=self.user
    ...
    def test_filter_by_ingredients(self):
        ...
        r1 = create_recipe(user=self.user, title='Pash Beans on Toast')
        r2 = create_recipe(user=self.user, title='Chicken Cacciotare')
